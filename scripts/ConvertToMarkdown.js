...
Usage: 
  ...
  Running the script:
    ...
    - Click Run button.
    - Converted doc will be saved on your google drive under "cookbook/src" and its images will be under "cookbook/img".
*/
...
function ConvertToMarkdown() {
  ...
  var cookBookFolder = !DriveApp.getFoldersByName("cookbook").hasNext() ? DriveApp.createFolder("cookbook") : DriveApp.getFoldersByName("cookbook").next();
  var imgFolder = !cookBookFolder.getFoldersByName("img").hasNext() ? cookBookFolder.createFolder("img") : cookBookFolder.getFoldersByName("img").next();
  var srcFolder = !cookBookFolder.getFoldersByName("src").hasNext() ? cookBookFolder.createFolder("src") : cookBookFolder.getFoldersByName("src").next();
  for (var i = 0; i < attachments.length; i++) {
    imgFolder.createFile(attachments[i].fileName, attachments[i].content, attachments[i].mimeType);
  }
  srcFolder.createFile(DocumentApp.getActiveDocument().getName() + ".md", text);
}
